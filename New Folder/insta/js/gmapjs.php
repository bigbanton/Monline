...
function infoBox() {
	...
	if (searchControls === q) {
		...
		e = e + '<input id="searchTxt" type="text" size="20" name="address" value="Enter a City or Address" />';
...
function doSearch($, Z) {
	...
	if ($.substr(0, 3) == "db:") {
		...
		//Adding parameters to find the distance between deal and the city///////////////////////////

		lngSpan = C - c,

        latSpan = I - R,

		point_lat = R + latSpan//,c + lngSpan * Math.random();
		...
		F = dbPath + "db.php?cat=" + B + "&swLat=" + R + "&swLng=" + c + "&neLat=" + I + "&neLng=" + C + "&extendLat=" + extendLat + "&extendLng=" + extendLng + "&point_lat=" + point_lat + "&point_lng=" + point_lng;
...
function findAddress(e, a) {
	...
	function (t, h) {
		...
		if (h == google.maps.GeocoderStatus.OK) {
			...
			startAddress = e;    ///////The value from the text field which i s
...
function OnLoad() {
	...
	//R = startAddress; original line of the code(downloaded one)        

	R = defaultAddress;     //error solved by Mr. Gajanand(analyst) changed from "R = startAddress;" to "R=defaultAddress"
