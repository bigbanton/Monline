				...
				if ( status == "success" || status == "notmodified" )
					...
					self.html( selector ?
						...
						jQuery("<div/>")
							...
							// to avoid any TEXT_EN_PERMISSION_DENIED_EN errors in IE
 ...
 * Written by Blair Mitchelmore (blair DOT mitchelmore AT google-email DOT com)
...

{
    ...
    jQuery('#pagecover').css({'position':'absolute', 'z-index':'3000', 'width':width, 'height':height, 'filter':'alpha(opacity=0.5)', 'opacity':0.5, 'top':0, 'left':0, 'background':'#CCC', 'display':display});
...
/*		return !jQuery('#sysmsg-error').remove();*/
	...
	jQuery('#consult-add-form input[name="commit"]').click(function(){
		...
		jQuery('#consult-add-form').ajaxSubmit({
			...
			'success' : function() { X.deals.consultation_again(); }
...
window.x_init_hook_imagerotate = function() {
	...
	var imagesPlay = function(next) { jQuery('#deals_images li').eq(imgListCurr).css({'opacity':'0.5'}).animate({'left':'-440px','opacity':'1'},'slow',function(){ jQuery(this).css({'left':'440px' }); }).end().eq(next).animate({'left':'0px','opacity':'1'},'slow',function(){ jQuery('#img_list a').siblings('a').removeClass('active').end().eq(next).addClass('active'); }); };
...
X.misc.noticenext = function(tid, nid) {
	...
	return X.get(BASE_REF + '/functions/ajax/manage.php?action=noticesubscribe&id='+tid+'&nid='+nid);
...
X.misc.noticesms= function(tid, nid) {
	...
	return X.get(BASE_REF + '/functions/ajax/manage.php?action=noticesms&id='+tid+'&nid='+nid);
...
X.misc.cardcode = function() {
	...
	if(oid&&cid) return !X.get(BASE_REF + '/functions/ajax/order.php?action=cardcode&id='+oid+'&cid='+cid);
...
X.coupon.dialogquery = function() {
	...
	if (id) return !X.get(BASE_REF + '/functions/ajax/coupon.php?action=query&id='+encodeURIComponent(id));
...
X.coupon.dialogconsume = function() {
	...
	if (id && secret) { 
		...
		return confirm(ask) && !X.get(BASE_REF + '/functions/ajax/coupon.php?action=consume&id='+encodeURIComponent(id)+'&secret='+encodeURIComponent(secret)); 
...
X.manage.loadtemplate = function(id) {
	...
	window.location.href = BASE_REF + '/manage/system/template.php?id='+id;

};

X.manage.loadpage = function(id) {

	window.location.href = BASE_REF + '/manage/system/page.php?id='+id;
...
X.manage.usermoney = function() {
	...
	if (uid&&money&&(!ask||confirm(ask))) return !X.get(BASE_REF + '/functions/ajax/manage.php?action=usermoney&id='+uid+'&money='+encodeURIComponent(money));
...
X.manage.orderexpress = function() {
	...
	if(oid) return !X.get(BASE_REF + '/functions/ajax/manage.php?action=orderexpress&id='+oid+'&eid='+eid+'&nid='+encodeURIComponent(nid));
...
X.manage.orderrefund = function() {
	...
	if(oid&&rid) return !X.get(BASE_REF + '/functions/ajax/manage.php?action=orderrefund&id='+oid+'&rid='+rid);

};

X.manage.dealscoupon = function(tid) {

	return !X.get(BASE_REF + '/functions/ajax/manage.php?action=dealscoupon&id='+tid);
